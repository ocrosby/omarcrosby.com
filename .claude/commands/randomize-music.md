...
Run the shuffle Python snippet in step 3 with `path.write_text` replaced by `print(head + "".join(rest)[:1200])` (or similar preview cap) and exit. Do not touch `data/music.yaml`, do not commit.

### 3. Shuffle entries 1..N

Run this one-shot Python (stdlib only), from the repo root:
...
if len(entries) < 3:
    print(f"only {len(entries)} entry/entries — nothing meaningful to shuffle", file=sys.stderr)
    raise SystemExit(1)
# Preserve the file preamble (comments and blank lines above the first entry).
preamble = text[: text.index("- youtube_id:")]
head, rest = entries[0], entries[1:]
random.shuffle(rest)
path.write_text(preamble + head + "".join(rest))
PY
```

The `preamble` slice keeps the two auto-generated header lines (`# Auto-generated by scripts/sync-recipes.py — do not edit by hand.`-style comments at the top of `data/music.yaml`) intact. `head` is entries[0] — the featured "now playing" video, pinned. `rest` is entries[1..N] — shuffled in place.
