...
This skill loads when writing or editing content under `content/` or `archetypes/`. The eight project rules — `content-frontmatter.md`, `theme-immutable.md`, `hugo-config-urls.md`, `external-link-hygiene.md`, `project-title-naming.md`, `per-post-og-image.md`, `per-song-og-image.md`, `now-page-updated-timestamp.md` — are the source of truth for enforcement. This skill provides the *how* for common authoring tasks.
