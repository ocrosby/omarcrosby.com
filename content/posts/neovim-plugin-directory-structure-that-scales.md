...
The tree tells you the *what* and the *when*. It doesn't tell you the *how* — the small handful of decisions embedded inside those files that determine whether the plugin ages well or gets rewritten in a year. The next post names those decisions and gives the recommended default for each, along with the pressure that makes that default the right one.
