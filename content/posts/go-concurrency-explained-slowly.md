...
**When you'd reach for it.** For very simple shared numbers or flags where a mutex is measurably too much overhead — a hit counter on a hot request path, a boolean "the shutdown has started" flag. The `sync/atomic` docs themselves say: *"Synchronization is better done with channels or the sync package."* Take that seriously. Atomics are a scalpel, not a first choice.
...
The primitives compose. When two or more primitives are used together in a recognizable shape often enough that the shape gets a name, that shape is called a **pattern**. This section is the shortest tour I could write of the ones you'll actually see.
...
The `context` package's docs spell out the discipline; internalize it once:
...
You don't need to memorize these to write correct code — following the practical rules above is enough — but knowing they exist means you can reason precisely when something surprising happens.
