...
- **Steps are bullets with a bold leading keyword** — `- **Given**`, `**When**`, `**Then**`, `**And**`, `**But**`. That bold-keyword convention is the entire syntax surface. It's a normal Markdown bullet list everywhere it's viewed; nothing about it requires a renderer that understands Lexicon specifically.
...
- **The `---`-delimited block at the top is front matter** for bookkeeping that has no BDD equivalent — a ticket ID, an owner, a status. It's kept deliberately separate from tags, so there isn't a second, parallel way to label a scenario.
...
`lexicon compile --to gherkin` on the pagination file above produces:
...
Every step carries both its surface `keyword` (`Given`/`When`/`Then`) and its resolved semantic `role` (`precondition`/`action`/`outcome`) as separate fields — the concept the original post actually argued for, made explicit instead of left for a reader to infer from BDD convention. That distinction is also the reason this format is worth a closer look if you're using an LLM anywhere in the loop.
...
That reads like a complete thought. It isn't — there's no `Then`. `lexicon check` doesn't care how fluent the prose is:
...
2. **`lexicon check` runs in CI** (there's a [GitHub Action](https://github.com/jedi-knights/lexicon/blob/main/action.yml) that wraps it) and catches structural gaps — a scenario with no `Then`, an empty Feature — before the story is ever picked up, regardless of who or what drafted it.
