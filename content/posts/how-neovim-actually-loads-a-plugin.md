...
The directory tree is a contract with Neovim. Get the names right, put the right kind of code in each one, and Neovim does the rest. The next post walks through how that tree evolves in practice — from the single-file plugin you built earlier, to the layered structure you'll see in any mature plugin, and what each new file earns you when you add it.
