...
The recipes RSS feed at `/recipes/index.xml` is generated by `layouts/recipes/list.rss.xml` at Hugo build time from `data/recipes.yaml` — no script step. New recipes appear in the feed automatically once they land in the yaml, but *only* if their `added` field is populated. Entries with `added: ""` are silently dropped by the template's defensive `where "added" "ne" ""` filter (`time.AsTime` would crash on empty otherwise), and the whole failure mode is invisible — the build succeeds and the missing entry is only noticed by a subscriber who expected to see it. `scripts/sync-recipes.py` guards against this via a fallback chain (git first-add → existing yaml value → today's date); never remove that fallback without providing an equivalent guarantee. See `.claude/rules/recipes-rss-feed.md`.
...
**`/verify` red is BLOCKING. Do not push if any of the four gates (Hugo build, lychee, markdownlint, typos) fails — including failures unrelated to the current diff.** Flagging a pre-existing failure in the PR body is not a substitute for fixing it: the PR's CI job will still be red, blocking merge and preventing semantic-release from cutting a version. If `/verify` shows red:

1. **Confirm whether the failure is caused by the current diff** (`git diff main --stat` — if the failing file isn't in the diff, it's pre-existing debt on `main`).
2. **If caused by the diff:** fix it before pushing. Standard case.
3. **If pre-existing:** stop and land a separate `fix(ci)` or `fix(content)` commit that resolves the failure first, then rebase and push the feature work. Do not push into a known-red state and hope the reviewer will sort it out — that pushes triage cost onto them and blocks the deploy chain for every subsequent PR until it's fixed anyway.
4. **If the user has explicitly authorized shipping into red CI** (rare, and only for genuinely-transient failures like a flaky upstream) — surface the failure clearly in the PR body and get explicit confirmation before pushing.

The failure mode this rule prevents: `/verify` catches a broken link on a project page unrelated to a new post, Claude flags it in the PR body, pushes anyway, and the PR sits with a red lychee check until someone remembers to open a separate fix PR — meanwhile blocking every downstream release.
...
- `.claude/rules/writing-voice.md` — concrete opening, early thesis, steelmanned opposing positions, attributed sources, a limitations section, and a one-sentence closing distillation — derived from reading existing posts. Applies when drafting or editing post prose.
